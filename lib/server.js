...
var require_store = __commonJS({
  "src/mission-control/store.js"(exports2, module2) {
    ...
    var REGISTRY_SNAPSHOT_KIND = "mission-control.registry.snapshot";
    var CARDS_SNAPSHOT_KIND = "mission-control.cards.snapshot";
    var EVENT_LOG_KIND = "mission-control.card-event";
    function getMissionControlStorePaths(dataDir) {
      const rootDir = path2.join(dataDir, "mission-control");
      return {
        rootDir,
        registrySnapshot: path2.join(rootDir, "registry.snapshot.json"),
        cardsSnapshot: path2.join(rootDir, "cards.snapshot.json"),
        eventLog: path2.join(rootDir, "card-events.jsonl")
      };
    }
    ...
    function atomicWriteJson(filePath, value) {
      ensureDirectory(path2.dirname(filePath));
...
`;
    ...
    function appendJsonLine(filePath, value) {
      ensureDirectory(path2.dirname(filePath));
      const fileDescriptor = fs2.openSync(filePath, "a");
      try {
        fs2.writeSync(fileDescriptor, `${stableStringify(value, 0)}
`, null, "utf8");
        fs2.fsyncSync(fileDescriptor);
      } finally {
    ...
    function migrateLegacySnapshot(rawSnapshot, kind) {
      if (kind === REGISTRY_SNAPSHOT_KIND) {
        const rawRegistry = rawSnapshot?.data && !Array.isArray(rawSnapshot.data) ? rawSnapshot.data : rawSnapshot;
        return createSnapshotEnvelope(
          kind,
          {
            ...rawRegistry || {},
            projects: (rawRegistry?.projects || []).map(
              (project) => normalizeProjectRegistryEntry(project, { now: rawRegistry?.updatedAt })
            )
          },
          {
      ...
      const rawCards = Array.isArray(rawSnapshot) ? rawSnapshot : Array.isArray(rawSnapshot?.cards) ? rawSnapshot.cards : Array.isArray(rawSnapshot?.data?.cards) ? rawSnapshot.data.cards : [];
      return createSnapshotEnvelope(
        kind,
    ...
    function readSnapshot(filePath, kind) {
      ...
      const rawSnapshot = JSON.parse(fs2.readFileSync(filePath, "utf8"));
      if (!rawSnapshot || typeof rawSnapshot !== "object") {
        throw new Error(`Mission Control snapshot ${filePath} is invalid JSON`);
      }
      if (rawSnapshot.schemaVersion === void 0) {
      ...
      if (kind === REGISTRY_SNAPSHOT_KIND) {
        return createSnapshotEnvelope(
          ...
          {
            ...rawSnapshot.data || {},
            projects: (rawSnapshot.data?.projects || []).map(
              (project) => normalizeProjectRegistryEntry(project, { now: rawSnapshot.data?.updatedAt })
            )
          },
          {
      ...
      return createSnapshotEnvelope(
        kind,
        {
          cards: (rawSnapshot.data?.cards || []).map(
            (card) => normalizeMasterCard(card, { now: rawSnapshot.writtenAt })
          )
        },
        {
          now: rawSnapshot.writtenAt,
    ...
    function upsertCard(cards, nextCard) {
      ...
      return [...withoutExisting, normalizedCard].sort(
        (left, right) => left.id.localeCompare(right.id)
      );
    }
    function applyEvent(state2, event) {
      switch (event.type) {
        case "registry.saved":
          return {
            ...state2,
        ...
        case "card.upserted":
          return {
            ...state2,
            cards: upsertCard(state2.cards, event.payload.card)
          };
        case "card.deleted":
          return {
            ...state2,
            cards: state2.cards.filter((card) => card.id !== event.payload.cardId)
          };
        default:
          return state2;
      }
    ...
    function replayMissionControlEvents(events, initialState = {}) {
      return (events || []).reduce((state2, event) => applyEvent(state2, event), {
    ...
    function createMissionControlStore(options = {}) {
      ...
      const registryState = replayMissionControlEvents(
        events.filter(
          (event) => event.type === "registry.saved" && event.sequence > (registrySnapshot?.lastEventSequence || 0)
        ),
        { registry: registrySnapshot?.data || null }
      );
      const cardsState = replayMissionControlEvents(
        events.filter(
          (event) => ["card.upserted", "card.deleted"].includes(event.type) && event.sequence > (cardsSnapshot?.lastEventSequence || 0)
        ),
      ...
      function saveRegistry(registry, saveOptions = {}) {
        ...
        const event = appendEvent(
          ...
          { sequence: state2.sequence + 1, now: saveOptions.now }
        );
      ...
      function writeCardsSnapshot(cards, saveOptions = {}) {
        writeSnapshot(
          ...
          {
            now: saveOptions.now,
            lastEventSequence: state2.sequence
          }
        );
      }
      function upsertMasterCard(card, saveOptions = {}) {
        ...
        const cards = upsertCard(state2.cards, normalizedCard);
        state2 = {
          ...state2,
          cards,
          sequence: event.sequence
      ...
      function deleteMasterCard(cardId, saveOptions = {}) {
        ...
        const cards = state2.cards.filter((card) => card.id !== normalizedCardId);
        state2 = {
          ...state2,
          cards,
          sequence: event.sequence
      ...
      if (options.registry && stableStringify(options.registry) !== stableStringify(state2.registry || {})) {
        saveRegistry(options.registry, { now: options.now });
      } else if (!state2.registry && options.registry) {
        saveRegistry(options.registry, { now: options.now });
      }
      if (!cardsSnapshot) {
        writeCardsSnapshot(state2.cards, { now: options.now });
      }
      function appendStoreEvent(event, appendOptions = {}) {
        const storedEvent = appendEvent(paths.eventLog, event, {
          sequence: state2.sequence + 1,
          now: appendOptions.now
        });
        state2 = {
          ...applyEvent(state2, storedEvent),
          sequence: storedEvent.sequence
        };
        if (["card.upserted", "card.deleted"].includes(storedEvent.type)) {
          writeCardsSnapshot(state2.cards, appendOptions);
        }
        if (storedEvent.type === "registry.saved") {
          writeSnapshot(paths.registrySnapshot, REGISTRY_SNAPSHOT_KIND, state2.registry, {
            now: appendOptions.now,
      ...
      return {
        ...
        deleteMasterCard,
        getCards: () => [...state2.cards],
        getRegistry: () => state2.registry,
        getState: () => ({
...
var require_mission_control = __commonJS({
  "src/mission-control/index.js"(exports2, module2) {
    var { loadMissionControlRegistry } = require_registry();
    var { createMissionControlStore } = require_store();
    function initializeMissionControl2(options = {}) {
      ...
      try {
        ...
        logger.log(
          `[Mission Control] Initialized ${state2.registry?.projectCount || 0} project(s), ${state2.cards.length} card(s)`
        );
        return {
          ready: true,
          store
    ...
    module2.exports = {
      initializeMissionControl: initializeMissionControl2
    };
  }
});
...
var { initializeMissionControl } = require_mission_control();
