  ...
  "src/mission-control/models.js"(exports2, module2) {
    ...
    function normalizeMasterCard(card, options = {}) {
      ...
      return {
        ...
        dispatchOwner: cleanNullableString(card?.dispatchOwner) || deriveDispatchOwner({
          lane,
          status,
          dispatch,
          humanReviewRequired,
    ...
    function createMasterCardFromLinearIssue(input, options = {}) {
      ...
      const derivedDependencies = childIssues.map((linkedIssue) => ({
        ...
        label: cleanString(linkedIssue.identifier) || cleanString(linkedIssue.title) || cleanString(linkedIssue.id),
        status: isTerminalLifecycle(normalizeLinearIssueLifecycle(linkedIssue)) ? "resolved" : "open",
        blocking: true
      }));
      const dependencies = (issue.dependencies || []).map(normalizeDependency).concat(derivedDependencies.map(normalizeDependency));
      ...
      const linkedIssueIds = uniqueSortedStrings(
        [primaryLinearIssueId].concat(issue.linkedIssueIds || []).concat(linkedIssues.map((linkedIssue) => linkedIssue.id)).filter(Boolean)
      );
      const linkedIssueIdentifiers = uniqueSortedStrings(
        [primaryLinearIdentifier].concat(issue.linkedIssueIdentifiers || []).concat(linkedIssues.map((linkedIssue) => linkedIssue.identifier)).filter(Boolean)
      );
      const linkedLinearProjectSlugs = uniqueSortedStrings(
        [project?.linearProjectSlug, issue.project?.slug, issue.projectSlug].concat(issue.linkedIssueProjectSlugs || []).concat(linkedIssues.map((linkedIssue) => linkedIssue.project?.slug)).filter(Boolean)
      );
...
`;
    function normalizeState(state2) {
      ...
      return {
        id: state2.id || null,
        name: state2.name || null,
        type: state2.type || null,
        color: state2.color || null
      };
    }
    ...
    function dedupeLinkedIssues(items) {
      ...
      for (const item of items) {
        ...
        byId.set(item.id, {
          ...existing,
          ...item,
          linkRole: nextRoles.has("child") ? "child" : nextRoles.has("parent") ? "parent" : existing.linkRole || item.linkRole || "related",
    ...
    function normalizeIssue(issue) {
      ...
      const childIssues = extractConnectionNodes(issue.children).map(
        (child) => normalizeLinkedIssueReference(child, "child")
      );
      const relatedIssues = extractConnectionNodes(issue.relations).map(
        (relation) => normalizeLinkedIssueReference(relation.relatedIssue, "related", relation.type || null)
      );
      const inverseRelatedIssues = extractConnectionNodes(issue.inverseRelations).map(
        (relation) => normalizeLinkedIssueReference(relation.issue, "related", relation.type || null)
      );
    ...
    function normalizeCard(input) {
      return {
        ...
        parentIssue: input.parentIssue ? {
          ...
          labels: Array.isArray(input.parentIssue.labels) ? input.parentIssue.labels.map((label) => ({
            id: label.id || null,
            name: label.name || null,
            color: label.color || null
          })).sort((left, right) => (left.name || "").localeCompare(right.name || "")) : [],
        ...
        linkedIssues: Array.isArray(input.linkedIssues) ? input.linkedIssues.map((issue) => ({
          id: issue.id || null,
          identifier: issue.identifier || null,
          title: issue.title || "Untitled",
          updatedAt: issue.updatedAt || null,
          ...
          (left, right) => (left.identifier || left.id || "").localeCompare(right.identifier || right.id || "")
        ) : [],
        linkedIssueIds: Array.isArray(input.linkedIssueIds) ? [...new Set(input.linkedIssueIds.filter(Boolean))].sort(
          (left, right) => left.localeCompare(right)
        ) : [],
        linkedIssueIdentifiers: Array.isArray(input.linkedIssueIdentifiers) ? [...new Set(input.linkedIssueIdentifiers.filter(Boolean))].sort(
          (left, right) => left.localeCompare(right)
        ) : [],
        linkedIssueProjectSlugs: Array.isArray(input.linkedIssueProjectSlugs) ? [...new Set(input.linkedIssueProjectSlugs.filter(Boolean))].sort(
          (left, right) => left.localeCompare(right)
        ) : [],
        cycle: input.cycle ? {
  ...
  "src/mission-control/linear/sync-engine.js"(exports2, module2) {
    ...
    function collectLinkedIssueIds(issues = []) {
      return [
        ...new Set(
          issues.flatMap(
            (issue) => [issue?.parentIssue?.id].concat(issue?.linkedIssueIds || []).concat((issue?.linkedIssues || []).map((linkedIssue) => linkedIssue?.id)).filter(Boolean)
          ).filter(Boolean)
        )
      ];
    }
      ...
      async function reconcile({ reason = "poll" } = {}) {
        ...
        reconcilePromise = (async () => {
          ...
          try {
            ...
            const supplementalIssues = supplementalIssueIds.length > 0 && typeof linearClient.fetchIssuesByIds === "function" ? await linearClient.fetchIssuesByIds({ issueIds: supplementalIssueIds }) : [];
            const issuesById = mergeIssueCollections(remoteIssues, supplementalIssues);
            const materializedIssues = [...issuesById.values()].map(
              (issue) => hydrateLinkedIssues(issue, issuesById)
            );
            let changedCount = 0;
...
var require_notifications = __commonJS({
  "src/mission-control/notifications.js"(exports2, module2) {
    ...
    function buildDiscordPayload(notification) {
      const color = notification.category === "completion" ? 4176208 : 16273737;
      ...
      function handleMissionControlChange(change, publicState) {
        if (change?.type === "card-upserted") {
          const card = (publicState?.masterCards || []).find(
            (entry) => entry.id === change.cardId || entry.primaryLinearIssueId === change.issueId || entry.primaryLinearIdentifier === change.identifier
          );
  ...
  "src/mission-control/index.js"(exports2, module2) {
    ...
    function createMissionControlService2({
      ...
      function resolveLaneOwner(card) {
        const explicitOwner = String(card?.responsibleAgents?.[0] || "").trim().toLowerCase();
        if (explicitOwner) {
      ...
      async function createCrossLaneChildTask(cardRef, input = {}) {
        ...
        const registryProject = registry.projects.find(
          (project) => project.key === targetProjectKey || project.linearProjectSlug === targetProjectSlug
        ) || null;
        ...
        const projectContext = typeof linearClient.resolveProjectBySlug === "function" ? await linearClient.resolveProjectBySlug(projectSlug) : null;
        ...
        const labelIds = typeof linearClient.resolveLabelIdsForTeam === "function" ? await linearClient.resolveLabelIdsForTeam({
        ...
        const publicState = getPublicState();
        const parentCard = publicState.masterCards.find((card) => card.id === reference.card.id) || null;
        const childCard = publicState.masterCards.find((card) => card.primaryLinearIssueId === createdIssue.id) || null;
        return {
          createdIssue,
          parentCard,
          childCard,
