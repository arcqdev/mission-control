...
var require_store = __commonJS({
  "src/mission-control/linear/store.js"(exports2, module2) {
    ...
    var SNAPSHOT_FILENAME = "linear-sync-snapshot.json";
    var EVENT_LOG_FILENAME = "linear-sync-events.jsonl";
    ...
    function createLinearSyncStore({
      ...
      const snapshotPath = path2.join(syncDir, SNAPSHOT_FILENAME);
      ...
      try {
        if (persistenceEnabled && fs2.existsSync(snapshotPath)) {
      ...
      function appendEvent(event) {
        ...
        if (!persistenceEnabled) return;
        try {
          fs2.appendFileSync(eventLogPath, `${JSON.stringify(event)}
`);
        } catch (error) {
          persistenceEnabled = false;
    ...
    module2.exports = { createLinearSyncStore, normalizeCard };
  }
});
...
var require_sync_engine = __commonJS({
  "src/mission-control/linear/sync-engine.js"(exports2, module2) {
    ...
    var { createLinearSyncStore } = require_store();
    function isoNow(now) {
      return new Date(now()).toISOString();
    }
    ...
    function isFreshTimestamp(timestamp, now, maxAgeMs = 5 * 60 * 1e3) {
...
var require_linear = __commonJS({
  "src/mission-control/linear/index.js"(exports2, module2) {
    var { createLinearClient, normalizeIssue } = require_client();
    var { createLinearSyncStore, normalizeCard } = require_store();
...
var require_registry = __commonJS({
  "src/mission-control/registry.js"(exports2, module2) {
    ...
    function normalizeSymphonyEndpoint(project) {
      ...
      const path2 = cleanString(project?.symphonyHealthPath || "/health") || "/health";
      return {
        url: `${protocol}//${host}:${port}${path2.startsWith("/") ? path2 : `/${path2}`}`,
        host,
        port,
        path: path2.startsWith("/") ? path2 : `/${path2}`,
        protocol
      };
    }
...
// src/mission-control/state-store.js
var require_state_store = __commonJS({
  "src/mission-control/state-store.js"(exports2, module2) {
    var fs2 = require("fs");
    var path2 = require("path");
    var SNAPSHOT_FILENAME = "mission-control-state.json";
    function isoNow(now = Date.now) {
      const value = typeof now === "function" ? now() : now;
      return new Date(value).toISOString();
    }
    ...
    function createMissionControlStateStore({ dataDir, now = Date.now, logger = console }) {
      ...
      try {
        ensureDir(rootDir);
        if (fs2.existsSync(snapshotPath)) {
          state2 = JSON.parse(fs2.readFileSync(snapshotPath, "utf8"));
        }
      } catch (error) {
      ...
      function getState() {
        return state2 ? JSON.parse(JSON.stringify(state2)) : null;
      }
      return {
        getState,
        update
...
var require_mission_control = __commonJS({
  "src/mission-control/index.js"(exports2, module2) {
    ...
    var { loadMissionControlRegistry, buildProjectRegistryIndexes } = require_registry();
    var { createSymphonyHealthProvider } = require_health_provider();
    var { createMissionControlStateStore } = require_state_store();
    ...
    function buildMissionControlPublicState({ linearState, registry, runtimeState, now = Date.now }) {
      ...
      const cardsByProjectKey = /* @__PURE__ */ new Map();
      ...
      const masterCards = (linearState.masterCards || []).map((card) => {
        ...
        const runtimeProject = registryProject ? runtimeByProjectKey.get(registryProject.key) || null : null;
      ...
      const lanes = Array.from(laneMap.values()).map((lane) => ({
        ...lane,
        status: lane.degradedProjectCount > 0 ? "degraded" : lane.highRiskCardCount > 0 ? "risk" : lane.staleCardCount > 0 ? "stale" : "ok"
    ...
    function createMissionControlService2({
      ...
      const linearSync = createLinearSyncEngine({
        ...
        onStateChange: (change) => {
          const publicState2 = refreshPublicState();
          onStateChange({ ...change, publicState: publicState2 });
        }
      });
      ...
      let publicState = stateStore.getState() || buildMissionControlPublicState({
...
var MISSION_CONTROL = createMissionControlService({
  config: CONFIG,
  dataDir: DATA_DIR,
  onStateChange: (change) => {
...
function handleMissionControlApi(req, res, pathname) {
  ...
  if (pathname === "/api/mission-control/admin/reconcile") {
    ...
    MISSION_CONTROL.reconcile({ reason: "manual-admin" }).then((nextState) => {
      if (typeof state.invalidateStateCache === "function") {
        state.invalidateStateCache();
      }
      writeJson(res, 200, {
