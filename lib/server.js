...
var require_models = __commonJS({
  "src/mission-control/models.js"(exports2, module2) {
    ...
    function createMasterCardFromLinearIssue(input, options = {}) {
      ...
      const dispatch = extractDispatchFromLabels(labelNames, null) || (/blocked/.test(cleanString(issue.state?.name).toLowerCase()) ? "dispatch:blocked" : null);
...
var require_mission_control = __commonJS({
  "src/mission-control/index.js"(exports2, module2) {
    ...
    function decorateMasterCard({ baseCard, linearCard, diagnostics, runtimeProject = null }) {
      const signals = [];
      const operationalRisk = runtimeProject?.symphony?.status === "unreachable" || diagnostics.stale || baseCard.risk === "risk:high" ? "high" : "low";
      ...
      return {
        ...
        healthStrip: {
          ...
          status: runtimeProject?.symphony?.status === "unreachable" || diagnostics.stale ? "degraded" : "healthy",
          signals
        },
        diagnostics
      };
    ...
    function buildMissionControlPublicState({
      ...
      const masterCards = (linearState.masterCards || []).map((linearCard) => {
        ...
        const runtimeProject = runtimeProjects.find(
          (entry) => entry.projectKey === project.key || entry.linearProjectSlug === project.linearProjectSlug
        );
        const diagnostics = buildCardDiagnostics({
          card: baseCard,
          sync: linearState.sync || {},
      ...
      const projects = (registry.projects || []).map((project) => {
        const projectCards = masterCards.filter(
          (card) => card.originProjects.includes(project.key) || card.project?.slug === project.linearProjectSlug
        );
        const runtimeProject = runtimeProjects.find(
          (entry) => entry.projectKey === project.key || entry.linearProjectSlug === project.linearProjectSlug
        );
        const degraded = runtimeProject?.symphony?.status === "unreachable" || projectCards.some((card) => card.healthStrip.stale);
        const signals = [];
        if (runtimeProject?.symphony?.status === "unreachable") {
          signals.push("symphony-down");
        }
      ...
      const lanes = [...new Set((registry.projects || []).map((project) => project.lane).filter(Boolean))].sort((left, right) => left.localeCompare(right)).map((lane) => {
