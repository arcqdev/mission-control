      ...
      function getFullState() {
        ...
        try {
          if (typeof getMissionControlState === "function") {
            missionControl2 = getMissionControlState();
          }
        } catch (e) {
        ...
        cachedState = {
          ...
          memory,
          cerebro,
          missionControl: missionControl2,
          subagents,
          pagination: {
            page: 1,
            pageSize: 20,
...
var require_store = __commonJS({
  "src/mission-control/linear/store.js"(exports2, module2) {
    ...
    function atomicWriteJson(filePath, value) {
      ...
      fs2.writeFileSync(tempPath, `${stableStringify(value, 2)}
`, "utf8");
      fs2.renameSync(tempPath, filePath);
    }
    function appendJsonLine(filePath, value) {
      ensureDir(path2.dirname(filePath));
      fs2.appendFileSync(filePath, `${stableStringify(value)}
`, "utf8");
    }
    ...
    function createLinearSyncStore({ dataDir, now = Date.now, pollIntervalMs, projectSlugs, webhook }) {
      ...
      function readEventLog() {
        if (!fs2.existsSync(eventLogPath)) {
          return [];
        }
        return fs2.readFileSync(eventLogPath, "utf8").split("\n").map((line) => line.trim()).filter(Boolean).map((line) => JSON.parse(line));
      }
...
function serveDocs(req, res) {
  ...
  const contentType = path.extname(resolvedPath).toLowerCase() === ".md" ? "text/markdown; charset=utf-8" : "text/plain; charset=utf-8";
...
var server = http.createServer((req, res) => {
  ...
  if (pathname === "/api/status") {
    ...
    missionControl.reconcile({ reason: "manual" }).then((syncState) => {
      res.writeHead(200, { "Content-Type": "application/json" });
    ...
    return;
  } else if (pathname.startsWith("/api/mission-control/cards/") && pathname.endsWith("/timeline")) {
  ...
  } else if (pathname.startsWith("/api/mission-control/cards/") && pathname.endsWith("/replay")) {
    const cardRef = decodeURIComponent(
